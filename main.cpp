...
class CustomCeresCallback : public ceres::IterationCallback {
    public:
        CustomCeresCallback(const ceres::Problem& _problem):
            problem(_problem) {
        ...
        virtual ceres::CallbackReturnType operator()(const ceres::IterationSummary& summary) {
            ...
            for(const auto& block : paramBlocks){
                ...
                Eigen::Map<Sophus::SE3d const> const pose(block);
                Eigen::Quaterniond so3(pose.so3().data());
                Eigen::AngleAxisd aa(so3);
...
struct CostFunctor {
    CostFunctor(const Eigen::Vector3d& target,
...
int main(int argc, char** argv){
    ...
    Eigen::Vector3d axis_groundtruth_inv;
    axis_groundtruth_inv << 0,0,1;
    Eigen::AngleAxisd aa_groundtruth_inv(-M_PI/3.0,axis_groundtruth_inv.normalized());
    Eigen::Quaterniond q_groundtruth_inv(aa_groundtruth_inv);

    Eigen::Vector3d t_groundtruth_inv;
    t_groundtruth_inv << -0.300,0.100,0.;

    Sophus::SE3d T_tmp(q_groundtruth_inv,t_groundtruth_inv);
    ...
    Eigen::Quaterniond q_groundtruth(T_groundtruth.so3().data());
    Eigen::AngleAxisd  aa_groundtruth(q_groundtruth);
    ...
    Eigen::MatrixXd sourcePoints(3,N);
    for(uint32_t i=0; i<N; i++){
        Eigen::Vector3d p = targetPoints.col(i);
        auto transP = T_tmp*p;
    ...
    for(uint32_t i=0; i<N; i++){
        Eigen::Vector3d target = targetPoints.col(i);
        Eigen::Vector3d input = sourcePoints.col(i);
    ...
    Eigen::Quaterniond q_out(T.so3().data());
    Eigen::AngleAxisd aa_out(q_out);
